...
public class Returned extends javax.swing.JFrame {
    ...
    private void tampilkan(){
        try{
            ...
            ps = con.prepareStatement("SELECT barang_balik.no_barang AS NoBarang, "
                    + "nama_barang AS NamaBarang, "
                    + "pembuat_laporan AS PembuatLaporan, "
                    + "nim_pembuat_laporan AS NIMPembuatLaporan, "
                    + "barang_balik.no_hp AS NoHP, "
                    + "waktu_laporan_dibuat AS WaktuLaporanDibuat "
                    + "FROM barang RIGHT JOIN barang_balik ON barang.no_barang = barang_balik.no_barang ");
            
            ResultSet rs = ps.executeQuery();
            tabelLaporan.setModel(DbUtils.resultSetToTableModel(rs));
        }
        catch(Exception ex){
    ...
    private void initComponents() {
        ...
        jLabel5.setText("LAPORAN BARANG KEMBALI");

        tabelLaporan.setFont(new java.awt.Font("Eras Light ITC", 1, 14)); // NOI18N
        tabelLaporan.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "no_barang", "nama_barang", "pembuat_laporan", "nim_pembuat_laporan", "no_hp", "waktu_laporan_dibuat"
            }
        ));
        ...
        jLabel7.setText("dan menghapus item dari list lost ataupun found");

        btnTerima.setFont(new java.awt.Font("Eras Light ITC", 1, 14)); // NOI18N
        ...
        jLabel8.setText("Pilih salah satu laporan dan Tekan tombol terima, untuk menerima laporan");
    ...
    private void tabelLaporanMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabelLaporanMouseClicked
        ...
        noBarang = (Integer) tabelLaporan.getValueAt(i, 0);
    ...
    private void btnTerimaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnTerimaActionPerformed
        ...
        try{
            con = DriverManager.getConnection("jdbc:mysql://localhost:3306/k05?user=root&password=root");
            ps = con.prepareStatement("UPDATE barang SET tipe = ? WHERE no_barang = ?");
            ps.setString(1, "Returned");
            ps.setString(2, String.valueOf(noBarang));
            ps.executeUpdate();
            ps = con.prepareStatement("DELETE FROM barang_balik WHERE no_barang = ?");
            ps.setString(1, String.valueOf(noBarang));
            ps.executeUpdate();
            con.close();
            JOptionPane.showMessageDialog(rootPane, "Laporan Telah Diterima");
            tampilkan();
        }
        catch(Exception ex){
            JOptionPane.showMessageDialog(rootPane, ex);
